...
export function ibanValidator(): ValidatorFn {
	return (control: AbstractControl): {[key: string]: any} | null => {
		const valid = isValidIBANNumber(control.value as string);
		return valid ? null : { 'invalid-iban': { value: control.value } };
	};
}
...
export function isValidIBANNumber(input: string) {
	...
	var CODE_LENGTHS: { [key: string]: number }    = {
		DE: 22,AD: 24, AE: 23, AT: 20, AZ: 28, BA: 20, BE: 16, BG: 22, BH: 22, BR: 29,
		CH: 21, CR: 21, CY: 28, CZ: 24,  DK: 18, DO: 28, EE: 20, ES: 24,
...
export function mod97(input: string): number {
	let checksum: number = parseInt(input.slice(0, 2));
	let fragment;
	for (let offset = 2; offset < input.length; offset += 7) {
		fragment = checksum.toString() + input.substring(offset, offset + 7);
